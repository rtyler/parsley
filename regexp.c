...
static void
exsltRegexpFlagsFromString (const xmlChar * flagstr, int *global, int *flags)
{
                    ...
                    int flags, int ovector[], int ovector_len)
{
  ...
  if (compiled_regexp == NULL)
    {
      xsltTransformError (xsltXPathGetTransformContext (ctxt), NULL, NULL,
                          "exslt:regexp failed to compile %s (char: %d). %s",
                          regexp, erroffset, error);
      return -1;
    }
  ...
  if (rc < -1)
    {
      xsltTransformError (xsltXPathGetTransformContext (ctxt), NULL, NULL,
                          "exslt:regexp failed to execute %s for %s", regexp,
                          haystack);
      rc = 0;
    }
          ...
          while (rc > 0)
            {
              for (int group = 0; group < rc; group++)
                {
                  match =
                    xmlStrsub (working, ovector[group * 2],
 ...
 */
void PLUGINPUBFUN
exslt_org_regular_expressions_init (void)
{
