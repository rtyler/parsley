...
FILE *
parsley_fopen (char *name, char *mode)
{
  FILE *fo;
  if (!strcmp ("-", name))
    {
...
parsley_apply_span_wrap (xmlDocPtr doc)
{
  if (span_wrap_sheet == NULL)
    {
      char *sheet =
        "<xsl:stylesheet version=\"1.0\" xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\" xmlns:sg=\"http://selectorgadget.com/\"> \
      ...
      xmlDocPtr xml =
        xmlCtxtReadMemory (ctxt, sheet, strlen (sheet), NULL, NULL, 0);
      span_wrap_sheet = xsltParseStylesheetDoc (xml);
    }
  xsltTransformContextPtr ctxt =
    xsltNewTransformContext (span_wrap_sheet, doc);
  xmlSetGenericErrorFunc (ctxt, parsleyXsltError);
  xmlDocPtr out =
    xsltApplyStylesheetUser (span_wrap_sheet, doc, NULL, NULL, NULL, ctxt);
...
{
  return (void
          *) (xmlNanoHTTPMethod (file_name, NULL, NULL, NULL,
                                 parsley_user_agent_header, 0));
}

void
parsley_io_set_mode (int mode)
{
  if (mode == parsley_io_mode)
    return;
...
xmlNodePtr
new_stylesheet_skeleton (char *incl)
{
  ...
  sprintbuf (buf, "%s",
             " extension-element-prefixes=\"lib str math set func dyn exsl saxon user date regexp regex\"");
  sprintbuf (buf, "%s", ">\n");
  sprintbuf (buf, "%s",
  ...
  sprintbuf (buf, "%s",
             "<func:function name=\"lib:nl\"><xsl:param name=\"in\" select=\".\"/>");
  sprintbuf (buf, "%s",
             "<xsl:variable name=\"out\"><xsl:apply-templates mode=\"innertext\" select=\"exsl:node-set($in)\"/></xsl:variable>");
  sprintbuf (buf, "%s", "<func:result select=\"$out\" /></func:function>");
  sprintbuf (buf, "%s",
             "<xsl:template match=\"text()\" mode=\"innertext\"><xsl:value-of select=\".\" /></xsl:template>");
  sprintbuf (buf, "%s",
             "<xsl:template match=\"script|style\" mode=\"innertext\"/>");
  sprintbuf (buf, "%s",
             "<xsl:template match=\"br|address|blockquote|center|dir|div|form|h1|h2|h3|h4|h5|h6|hr|menu|noframes|noscript|p|pre|li|td|th|p\" mode=\"innertext\"><xsl:apply-templates mode=\"innertext\" /><xsl:text>\n</xsl:text></xsl:template>");
  sprintbuf (buf, "%s\n", incl);
  sprintbuf (buf, "%s\n", "<xsl:template match=\"/\">\n");
  sprintbuf (buf, "%s\n", "<parsley:root />\n");
  sprintbuf (buf, "%s\n", "</xsl:template>\n");
  sprintbuf (buf, "%s\n", "</xsl:stylesheet>\n");
  xmlParserCtxtPtr ctxt = xmlNewParserCtxt ();
  xmlDocPtr doc = xmlCtxtReadMemory (ctxt, buf->buf, buf->size,
                                     "http://parselets.com/compiled", NULL,
                                     3);
