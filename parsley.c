...
void
parsed_parsley_free (parsedParsleyPtr ptr)
{
  if (ptr->xml != NULL)
    xmlFree (ptr->xml);
  if (ptr->error != NULL)
...
parse_error (char *format, ...)
{
  parsedParsleyPtr ptr =
    (parsedParsleyPtr) calloc (sizeof (parsed_parsley), 1);
  ptr->xml = NULL;
  va_list args;
...
parsedParsleyPtr
parsley_parse_file (parsleyPtr parsley, char *file, int flags)
{
  ...
  if (html)
    {
      ...
      htmlDocPtr html = htmlCtxtReadFile (htmlCtxt, file, encoding,
                                          HTML_PARSE_RECOVER |
                                          HTML_PARSE_NOERROR |
                                          HTML_PARSE_NOWARNING);
      htmlFreeParserCtxt (htmlCtxt);
      if (html == NULL)
        return parse_error ("Couldn't parse file: %s\n", file);
  ...
  else
    {
      ...
      xmlDocPtr xml = xmlCtxtReadFile (ctxt, file, encoding,
...
parsedParsleyPtr
parsley_parse_string (parsleyPtr parsley, char *string, size_t size,
                      char *base_uri, int flags)
{
  xmlSetGenericErrorFunc (NULL, parsleyXsltError);
  bool html = flags & PARSLEY_OPTIONS_HTML;
  char *encoding = flags & PARSLEY_OPTIONS_FORCE_UTF8 ? "UTF-8" : NULL;
  if (base_uri == NULL)
    base_uri = "http://parselets.com/in-memory-string";
  if (html)
    {
      htmlParserCtxtPtr htmlCtxt = htmlNewParserCtxt ();
      htmlDocPtr html =
        htmlCtxtReadMemory (htmlCtxt, string, size, base_uri, encoding,
                            HTML_PARSE_RECOVER | HTML_PARSE_NOERROR |
                            HTML_PARSE_NOWARNING);
      if (html == NULL)
        return parse_error ("Couldn't parse string");
      parsedParsleyPtr out = parsley_parse_doc (parsley, html, flags);
      xmlFreeDoc (html);
      return out;
    }
  else
    {
      xmlParserCtxtPtr ctxt = xmlNewParserCtxt ();
      xmlDocPtr xml =
        xmlCtxtReadMemory (ctxt, string, size, base_uri, encoding,
                           HTML_PARSE_RECOVER | HTML_PARSE_NOERROR |
                           HTML_PARSE_NOWARNING);
      if (xml == NULL)
        return parse_error ("Couldn't parse string");
...
{
  if (node == NULL || node->name == NULL || node->parent == NULL)
    return strdup ("/");
...
is_root (xmlNodePtr node)
{
  return node != NULL && node->ns != NULL
    && !strcmp (node->ns->prefix, "parsley") && !strcmp (node->name, "root");
}
...
collate (xmlNodePtr xml)
{
  ...
  if (xml->type != XML_ELEMENT_NODE)
    return;
  if (xml->ns != NULL && !strcmp (xml->ns->prefix, "parsley")
      && !strcmp (xml->name, "zipped"))
    {
      ...
      for (j = 0; j < len; j++)
        {
          ...
          if (j == 0 || (!empty[i] && !multi[i] && !optional[i]))
            {                   // first or full
              ...
              for (int k = 0; k < n; k++)
                {
                  ...
                  if (multi[k])
                    targets[k] =
                      xmlNewChild (targets[k], xml->ns, "groups", NULL);
                }
            }

          if (!multi[i])
            sortable[j] = sortable[j]->children;
          if (empty[i] || multi[i])
...
prune (parsedParsleyPtr ptr, xmlNodePtr xml, char *err)
{
  ...
  if (optional)
    ...
    {
      if (err == NULL)
        asprintf (&err, "%s was empty", xpath_of (xml));
...
parsedParsleyPtr
parsley_parse_doc (parsleyPtr parsley, xmlDocPtr doc, int flags)
{
  killDefaultNS (doc);
  ...
  xsltTransformContextPtr ctxt =
    xsltNewTransformContext (parsley->stylesheet, doc);
  ...
  ptr->xml =
    xsltApplyStylesheetUser (parsley->stylesheet, doc, NULL, NULL, NULL,
                             ctxt);
  ...
  if (ptr->error == NULL)
    {
      if (ptr->xml != NULL && ptr->error == NULL)
        {
          if (flags & PARSLEY_OPTIONS_COLLATE)
            collate (ptr->xml->children);
          if (flags & PARSLEY_OPTIONS_PRUNE)
...
json_invalid_object (parsleyPtr ptr, struct json_object *json)
{
  json_object_object_foreach (json, key, val)
  {
    if (val == NULL)
      ptr->error =
        strdup
        ("Parselets can only be made up of strings, arrays, and objects.");
      ...
      case json_type_array:
        ...
        switch (json_object_get_type (inner))
          ...
          case json_type_object:
            if (json_invalid_object (ptr, inner))
              return true;
            break;
          default:
            ptr->error =
              strdup
              ("Arrays may contain either a single string or an object.");
            return true;
          }
        break;
      case json_type_object:
...
parsleyPtr
parsley_compile (char *parsley_str, char *incl)
{
  ...
  if (is_error (json))
    {
      asprintf (&(parsley->error),
                "Your parselet is not valid json: %s at char:%d",
                json_tokener_errors[-(unsigned long) json], error_offset);
      return parsley;
    }
...
contextPtr
deeper_context (contextPtr context, char *key, struct json_object * val)
{
...
yyerror (const char *s)
{
  ...
  if (last_parsley_error != NULL)
    sprintbuf (buf, "%s\n", last_parsley_error);
  sprintbuf (buf, "%s in key: %s", s,
             full_key_name (parsley_parsing_context));
...
inner_path (pxpathPtr p)
{
  if (p == NULL)
    return NULL;
  if (p->type == PXPATH_PATH)
    return pxpath_to_string (p);
  char *tmp = NULL;
...
{
  return inner_path_transform (c) ? inner_path (c->expr) :
    pxpath_to_string (c->filter);
}

static char *
resolve_expr (contextPtr c)
{
  return inner_path_transform (c) ? inner_path_to_dot (c->expr) :
...
render (contextPtr c)
{
  ...
  if (c->array)
    c->node = xmlNewChild (c->node, parsley, "groups", NULL);
  if (filtered)
    {
  ...
  if (multiple)
    {
      ...
      if (!(c->flags & PARSLEY_BANG))
        xmlSetProp (c->node, "optional", "true");
    }
  ...
  if (c->string)
    ...
    {
      if (magic_children)
        c->node = xmlNewChild (c->node, parsley, "zipped", NULL);
      __parsley_recurse (c);
    }
...
void
__parsley_recurse (contextPtr context)
{
  contextPtr c;
...
static char *
inner_key_of (struct json_object *json)
{
