...
pxpathPtr
pxpath_new (int type, char *value)
{
...
char *
pxpath_to_string (pxpathPtr ptr)
{
  if (ptr == NULL)
    return NULL;
...
pxpathPtr
pxpath_new_func (char *value, pxpathPtr child)
{
...
pxpathPtr
pxpath_cat_things (int n, va_list va)
{
  struct printbuf *buf = printbuf_new ();
  pxpathPtr last;
  pxpathPtr first;
